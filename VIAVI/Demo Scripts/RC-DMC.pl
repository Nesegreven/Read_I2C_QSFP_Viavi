###############################################################################
## RC-DMC.pl
## Copyright (c) 2011 JDSU
## This script is provided "as-is" and is meant for demonstration purposes only.
##
## Establishes a remote control connection to an MSAM module in a 
## T-BERD/MTS 8000 Dual Module Carrier (DMC).
##
## Syntax:
...
##  <slice #> - optional, specifies which transport module to communicate with
...
my $side = shift @ARGV || "OPPS";
my $sliceNum = shift @ARGV || 1;

# SLIC1 - first "slice", closest module to the front display
# SLIC2, SLIC3 - second and third slices
my $slice = "SLIC$sliceNum";
# Accept either "1" or "SLIC1" as an argument
$slice = $sliceNum if ($sliceNum =~ /SLIC\d/);
...
sub connectToRC {
    ...
    # this is used several times in the first 2 steps... something like BOTH,SLIC1,"BERT"
